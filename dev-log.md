...
2026-06-13: Allocations diff-completion job. Computed full expected set from seed_2026.json = 779 alloc keys (project sys_id|team sys_id|month), fte sum 386.75; project match keyed on name+initiatives_group (handles duplicate names e.g. C4U Migration). Queried loaded set (560) → MISSING=219. 0 unmapped projects, 0 unmapped teams. Inserted 216 net-new via MCP create_record; remaining 3 (project 4083dd…/"Renewals Process Automation (Phase 1)": BA m2 0.5, PM m1 0.5, PM m2 0.5) returned 403 — confirmed via query these 3 already existed (BR-01 dedup abort surfacing as 403), so treated as skip per spec. Final verify: total allocation count year 2026 = 779, fte sum = 386.75. COMPLETE (779).
